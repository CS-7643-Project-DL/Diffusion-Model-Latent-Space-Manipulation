...
def latent_space_manipulation(latents, noised_latent_t, cumulative_attention_maps, zz, focus):
    for subject, idx in cumulative_attention_maps.items():
        # Generate mask
        if subject == focus:
            mask = generate_mask(cumulative_attention_maps[subject], zz, True)
        ...
        # Replace values in latents with corresponding values from noised_latent_t
            plt.imshow(noised_latent_t[80].squeeze().cpu().numpy().transpose(1,2,0))   
            plt.show()
            for idx in zero_indices:
                noise = noised_latent_t[80]
                latents[0, :, idx[2], idx[3]] = noise[0, :, idx[2], idx[3]]
            for idx in one_indices:
                noise = noised_latent_t[680]
                latents[0, :, idx[2], idx[3]] = noise[0, :, idx[2], idx[3]]
        ...
        # Find indices where mask is 0
            ...
            one_indices = (mask != 0).nonzero()
        # Replace values in latents with corresponding values from noised_latent_t
            plt.imshow(noised_latent_t[680].squeeze().cpu().numpy().transpose(1,2,0))
            plt.show()
            for idx in zero_indices:
                noise2 = noised_latent_t[680]
                latents[0, :, idx[2], idx[3]] = noise2[0, :, idx[2], idx[3]]
            for idx in one_indices:
                noise = noised_latent_t[120]
                latents[0, :, idx[2], idx[3]] = noise[0, :, idx[2], idx[3]]
...
def timestamps_to_manipulate(sampler):
    ...
    preserve_timesteps = [sampler.timesteps[34], sampler.timesteps[30], sampler.timesteps[28], sampler.timesteps[0]]
    edit_timesteps = [sampler.timesteps[2],sampler.timesteps[4]]
